...
void iterate(int clientfd, thread_arg *targ)
{
    ...
    char c = START_LETTER + targ->id -1;
    if(send(clientfd, (char*)&c,sizeof(char) , 0) < 0) ERR("send");
    fprintf(stderr,"sent %c",c);
    ssize_t size;
    while(c != START_LETTER + targ->id)
    {
        if((size = recv(clientfd, (char*)&c,sizeof(char) , 0) )<0) ERR("recv");
